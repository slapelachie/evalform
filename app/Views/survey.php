            ...
            <?php foreach ($questions as $question) : ?>
                <div class="mb-3">
                    <?php if ($question['type'] == 'multiple_choice') { ?>
                        ...
                        <?php foreach ($question['choices'] as $choice) : ?>
                            <div class="form-check mb-3">
                                ...
                                <input type="radio" name="question-<?= $question['question_number'] ?>" class="form-check-input" id="question-<?= $question['question_number'] . '-' . $choice['position'] ?>">
                            </div>
                        <?php endforeach; ?>
                    <?php } elseif ($question['type'] == 'free_text') { ?>
                        ...
                        <textarea name="question-<?= $question['question_number'] ?>" id="question-<?= $question['question_number'] ?>" rows="3" class="form-control"></textarea>
                    <?php } ?>
                </div>
            <?php endforeach; ?>
